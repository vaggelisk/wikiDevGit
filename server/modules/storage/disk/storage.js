...
module.exports = {
  ...
  async assetUploaded (asset) {
    ...
    WIKI.logger.info(`(STORAGE/DISK) VAGGELIS...  ${this.config.path}`)


    await fs.outputFile(path.join(this.config.path, asset.path), asset.data)
      .then(
        // edw thelei ena if
        // isws to if na einai oti to asset.path xekinaei me pdf-redesign (isws)
        exec(`cp ${this.config.path}/${asset.path}  ${this.config.path}/${asset.path}.jfdhfjhfj`)
      )
  },
   ...
   * RUN A COMMAND
   */
  function : runCommand = async ()=> {
    console.log("Hello ")
  },
  // async runCommand ( ) {
  //   return  WIKI.logger.info(`(STORAGE/DISK) VAGGELIS2...`)
  // },
